...
|Ключ|Полное название|Описание|
|---|---|---|
|**`-c`**|`--create`|Создать новый архив|
|**`-x`**|`--extract`|Распаковать (извлечь) файлы из архива|
|**`-t`**|`--list`|Показать содержимое архива без его распаковки|
|**`-f`**|`--file`|Указать имя файла архива (обязателен, ставится последним перед именем файла)|
|**`-v`**|`--verbose`|Выводить список обрабатываемых файлов в терминал (ход работы)|
|**`-z`**|`--gzip`|Сжать или распаковать архив с помощью алгоритма **gzip** (`.tar.gz`, `.tgz`)|
|**`-j`**|`--bzip2`|Сжать или распаковать архив с помощью алгоритма **bzip2** (`.tar.bz2`)|
|**`-J`**|`--xz`|Сжать или распаковать архив с помощью алгоритма **xz** (`.tar.xz`)|
|**`-C`**|`--directory`|Сменить папку перед началом операции (например, для распаковки в конкретный путь)|
|**`-r`**|`--append`|Добавить файлы в конец уже существующего архива (только для несжатых `.tar`)|
...
## `scp` Копирование файлов между серверами по сети
```
scp /opt/distr/название_файла имя_пользователя@IP_адрес:/путь/куда/копировать/
```

| Ключ     | Полное название | Описание                                                                         |
| -------- | --------------- | -------------------------------------------------------------------------------- |
| **`-r`** | `--recursive`   | Рекурсивное копирование (обязателен для отправки целых папок)                    |
| **`-P`** | `--port`        | Указать порт подключения к SSH (регистр важен, `P` должна быть заглавной)        |
| **`-C`** | `--compress`    | Включить сжатие данных «на лету» (ускоряет передачу текстовых файлов)            |
| **`-q`** | `--quiet`       | Скрыть индикатор прогресса, отчеты и предупреждения (тихий режим)                |
| **`-v`** | `--verbose`     | Выводить подробную отладочную информацию (полезно при поиске ошибок подключения) |
| **`-p`** | `--preserve`    | Сохранить время модификации, время доступа и права (permissions) исходных файлов |
| **`-i`** | `--identity`    | Указать путь к приватному SSH-ключу для авторизации (если вход не по паролю)     |
| **`-l`** | `--limit`       | Ограничить скорость передачи данных (задается в кбит/с, например `-l 1000`)      |
| **`-4`** | `--ipv4`        | Использовать только протокол IPv4                                                |
| **`-6`** | `--ipv6`        | Использовать только протокол IPv6                                                |
## `template` Описание
